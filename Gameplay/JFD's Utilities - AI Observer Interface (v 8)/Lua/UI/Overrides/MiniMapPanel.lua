...
function OnMiniMapOverview()
	LuaEvents.JFD_UI_ShowMiniMapOverview()
end

Controls.OverlayMapsButton:RegisterCallback(Mouse.eLClick, OnMiniMapOverview);
----------------------------------------------------------------
----------------------------------------------------------------
...
function UpdateStrategicViewToggleTT()
	...
	if (controlInfo ~= nil and type(controlInfo) == "table") then
		...
		if (hotKey ~= nil and type(hotKey) == "string") then
			local keyDesc = Locale.GetHotKeyDescription(hotKey, controlInfo.CtrlDown, controlInfo.AltDown,
				controlInfo.ShiftDown);
