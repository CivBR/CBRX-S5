...
g_LegendBarbPlayerInstanceManager = InstanceManager:new("PlayerInstance", "PlayerButton", Controls
.LegendBarbPlayersStack);
g_LegendMinorPlayerInstanceManager = InstanceManager:new("PlayerInstance", "PlayerButton",
	Controls.LegendMinorPlayersStack);
g_LegendReligionInstanceManager = InstanceManager:new("ReligionInstance", "ReligionButton", Controls
.LegendReligionsStack);
...
function PopulateSortedPlayers()
	...
	local sActivePlayerDesc = Locale.ConvertTextKey("{Civ} ([COLOR_POSITIVE_TEXT]YOU[ENDCOLOR])",
		pActivePlayer:GetCivilizationShortDescription())
...
function PopulateRankedPlayers(overlayMapID)
	...
	for numRank, tPlayer in ipairs(g_SortedRankedPlayerEntryTable) do
		...
		g_PlayerLegendsTable[iPlayer].RankInfo = { Rank = numRank, RankTotal = numRankTotal, RankVal = numRankVal, RankHighestVal =
		numRankHighestVal }
	end
end
...
function GetColour(overlayMapID, thisLegend, thisLegendInfo, iPlayer, iReligion, pPlot)
	local overlayMap = GameInfo.JFD_OverlayMaps[overlayMapID]
	local overlayMapType = overlayMap.Type
	local overlayLegend = GameInfo.JFD_OverlayMap_Legends("OverlayMapType = '" ..
	overlayMapType .. "' AND LegendType = '" .. thisLegend .. "'")()
...
function GetColourForCityPlot(overlayMapID, iPlayer, pCity)
	...
	if (optionShowBorders.Check and optionShowBorders.IsValid) or overlayMapType == "OVERLAY_MAP_JFD_PLAYERS_BORDERS_OWNER" then
		if pPlayer:IsMinorCiv() then
			...
			colour = { Red = (255 * playerColour.x), Green = (255 * playerColour.y), Blue = (255 * playerColour.z), Alpha =
			playerColour.w }
		else
			local playerColour, _ = pPlayer:GetPlayerColors()
			colour = { Red = (255 * playerColour.x), Green = (255 * playerColour.y), Blue = (255 * playerColour.z), Alpha =
		...
		for y = 0, mapHeight - 1, 1 do
			for x = 0, mapWidth - 1, 1 do
				...
				if pPlot then
					...
					Controls.Map:SetPlot(x, y, pPlot:GetTerrainType(), colour.Red / 255, colour.Green / 255,
						colour.Blue / 255, colour.Alpha)
				end
			end
		end
	end
			...
			if (thisLegendPlayer and thisLegendPlayer.Checked) then
				if overlayMapOptionShowBorders and pOtherPlayer:GetNumCities() > 0 then
					...
					for pCity in pOtherPlayer:Cities() do
						...
						if (isActiveTeam and pPlot:IsRevealed(iActiveTeam)) or (not isActiveTeam) then
							ShadeCityTiles(iActiveTeam, pCity, colour, colourA, (not overlayMapOptionShowCities),
								(not overlayMapOptionExcWaterTiles))
						end
					end
				end
					...
					for pCity in pOtherPlayer:Cities() do
						...
						if pPlot then
							if (isActiveTeam and pPlot:IsRevealed(iActiveTeam)) or (not isActiveTeam) then
								...
								local colourA = colour.Alpha
								Controls.Map:SetPlot(pPlotX, pPlotY, pPlot:GetTerrainType(), colour.Red / 255,
									colour.Green / 255, colour.Blue / 255, colourA)
							end
						end
					end
				end
			...
			for x = 0, mapWidth - 1, 1 do
				...
				if (isActiveTeam and pPlot:IsRevealed(iActiveTeam)) or (not isActiveTeam) then
					...
					if iTerrain ~= iTerrainOcean and iTerrain ~= iTerrainCoast and pPlot:GetFeatureType() ~= iFeatureIce then
						local thisLegend = GetOverlayMapLegend(overlayMapID, overlayMapFilterID, iActivePlayer,
							iActiveTeam, iOtherPlayer, iOtherTeam, pCity, pPlot, pUnit, iReligion)
						if thisLegend then
							...
							if thisLegendInfo then
								...
								if colour then
									if thisLegendInfo.Checked then
										Controls.Map:SetPlot(x, y, pPlot:GetTerrainType(), colour.Red / 255,
											colour.Green / 255, colour.Blue / 255, colourA)
									end
							...
							if iOtherPlayer > -1 then
								...
								if pOtherPlayer:IsAlive() then
									...
									if ((isActiveTeam and pOtherTeam:IsHasMet(iActiveTeam)) or (not isActiveTeam)) then
										local thisLegendPlayer = g_PlayerLegendsTable[iOtherPlayer]
										if (thisLegendPlayer and thisLegendPlayer.Checked) then
											local thisLegend = GetOverlayMapLegend(overlayMapID, overlayMapFilterID,
												iActivePlayer, iActiveTeam, iOtherPlayer, iOtherTeam, pCity, pPlot, pUnit,
												iReligion)
											if thisLegend then
												local thisLegendInfo = g_ActiveOverlayMap[overlayMapType][thisLegend]
												if thisLegendInfo then
													local colour = GetColour(overlayMapID, thisLegend, thisLegendInfo,
														iOtherPlayer)
													local colourA = colour.Alpha
													if thisLegendInfo.Checked then
														Controls.Map:SetPlot(x, y, pPlot:GetTerrainType(), colour.Red /
														255, colour.Green / 255, colour.Blue / 255, colourA)
													end
													if (not thisLegendInfo.Colour) or thisLegendInfo.Colour ~= colour then
														thisLegendInfo.Colour = colour
													end
	...
	elseif overlayMapIteratePlayers then
		for i, tPlayer in ipairs(playersTable) do
			...
			if (thisLegendPlayer and thisLegendPlayer.Checked) then
				...
				local thisLegend = GetOverlayMapLegend(overlayMapID, overlayMapFilterID, iActivePlayer, iActiveTeam,
					iOtherPlayer, iOtherTeam, pCity, pPlot, pUnit, iReligion)
				if thisLegend then
					local thisLegendInfo = g_ActiveOverlayMap[overlayMapType][thisLegend]
					if thisLegendInfo then
						local colour = GetColour(overlayMapID, thisLegend, thisLegendInfo, iOtherPlayer)
...
function ShadeCityTiles(iActiveTeam, pCity, colour, colourA, isShadeCityTile, isIncludeWaterTiles)
			...
			if (pPlot:GetOwner() == pCity:GetOwner()) then
				if (((not pPlot:IsWater()) and (not isIncludeWaterTiles)) or isIncludeWaterTiles) then
					if (pPlot:IsMountain() or pPlot:GetTerrainType() == iTerrainDesert or pCity:IsWorkingPlot(pPlot) or pCity:CanWork(pPlot)) then
						Controls.Map:SetPlot(pPlot:GetX(), pPlot:GetY(), pPlot:GetTerrainType(), colour.Red / 255,
							colour.Green / 255, colour.Blue / 255, colourA)
					end
				end
			end
	...
	if isShadeCityTile then
		...
		if pPlot and (isActiveTeam and pPlot:IsRevealed(iActiveTeam)) or (not isActiveTeam) then
			Controls.Map:SetPlot(pPlotX, pPlotY, pPlot:GetTerrainType(), colour.Red / 255, colour.Green / 255,
				colour.Blue / 255, colourA)
		end
	end
end
	...
	for i, tPlayer in ipairs(g_SortedRankedPlayerEntryTable) do
		...
		if pPlayer:IsAlive() and ((isActiveTeam and pTeam:IsHasMet(iActiveTeam)) or (not isActiveTeam)) then
			...
			if thisPlayerInfo and thisPlayerInfo.Visible and thisLegendInfo and (thisLegendInfo.Visible) then
				...
				local strShortDesc = Locale.ConvertTextKey(legend.ShortDescription, strPlayerShortDesc, strPlayerRank,
					strLeaderShortDesc)
				local strRankShortDesc = Locale.ConvertTextKey(legendRanking.ShortDescription, legendRanking.IconFont,
					strPlayerRankVal)
				...
				local strColour = "[COLOR:" ..
				255 .. ":" .. legendColourG .. ":" .. legendColourB .. ":" .. legendColourA .. "]"
				legendInstance.PlayerLine:SetColorVal(legendColourR / 255, legendColourG / 255, legendColourB / 255,
					legendColourA);
				legendInstance.PlayerLine:SetHide(false)

				local strRankDesc = Locale.ConvertTextKey(legendRanking.Description, strColour, strPlayerRank,
					strPlayerShortDesc, strPlayerRankVal, legendRanking.IconFont)
				...
				legendInstance.ShowHide:RegisterCheckHandler(function(bCheck)
					...
					ShadeMap(overlayMapID, overlayMapFilterID, overlayMapFilterPlayerID, overlayMapFilterReligionID,
						legendGroupID)
				end);
			end
		end
...
function PopulateOverlayPlayers(overlayMapID)
	...
	for i, tPlayer in ipairs(g_SortedMajorPlayerEntryTable) do
		...
		if pPlayer:IsAlive() and (not pPlayer:IsMinorCiv()) and (not pPlayer:IsBarbarian()) and ((isActiveTeam and pTeam:IsHasMet(iActiveTeam)) or (not isActiveTeam)) then
			...
			local thisLegendInfo = tableMapOverlay[thisLegend]
			if thisPlayerInfo and thisPlayerInfo.Visible and (thisLegendInfo and thisLegendInfo.Visible) then
				local legend = GameInfo.JFD_OverlayMap_Legends("OverlayMapType = '" ..
				overlayMapType .. "' AND LegendType = '" .. thisLegend .. "'")()
				local legendInstance = g_LegendPlayerInstanceManager:GetInstance();
				...
				local legendColour = thisPlayerInfo.Colour
				legendInstance.PlayerLine:SetColorVal(legendColour.Red / 255, legendColour.Green / 255,
					legendColour.Blue / 255, legendColour.Alpha);
				legendInstance.PlayerLine:SetHide(false)
			...
			if pPlayer:HasCreatedReligion() then
				...
				if pPlayer:IsAlive() and ((isActiveTeam and pTeam:IsHasMet(iActiveTeam)) or (not isActiveTeam)) then
					...
					if thisReligionInfo and thisReligionInfo.Visible and (thisLegendInfo and thisLegendInfo.Visible) then
						...
						legendInstance.ReligionLine:SetColorVal(legendColour.Red / 255, legendColour.Green / 255,
							legendColour.Blue / 255, legendColour.Alpha);
	...
	for row in GameInfo.JFD_OverlayMap_Legends("OverlayMapType ='" .. overlayMapType .. "'") do
		...
		if addLegend then
			...
			if (thisLegendInfo.Visible or row.ShowInLegendAlways) then
				...
				legendInstance.LegendLine:SetColorVal(legendColour.Red / 255, legendColour.Green / 255,
					legendColour.Blue / 255, legendColour.Alpha);
...
function PopulateOverlayMapOptions(overlayMapID)
	...
	for row in GameInfo.JFD_OverlayMapOptions() do
		...
		local isValid = GameInfo.JFD_OverlayMap_Options("OverlayMapType = '" ..
		overlayMapType .. "' AND OptionType = '" .. optionType .. "'")()
		...
		if isValid then
			...
			function OnShowHideOptionBarbs(bCheck)
				...
				if optionIncludeBarbs.Check and optionIncludeBarbs.IsValid then
					...
					Controls.BarbPlayersButton:SetHide(false)
					-- Controls.BarbPlayerGrid:SetHide(not isShowPlayers)
					Controls.BarbPlayersLabel:SetText("[ICON_PLUS]" ..
					Locale.ConvertTextKey("TXT_KEY_OVERLAY_JFD_BARBARIANS"));
				else
					Controls.BarbPlayersButton:SetHide(true)
					Controls.BarbPlayerGrid:SetHide(true)
				end
...
function RefreshOverlayMapFilterPlayer(overlayMapClassID, overlayMapFilterPlayerID)
	...
	if overlayMapFilterPlayerID ~= -1 then
		...
		Controls.OverlayMapFilterPlayerPullDown:GetButton():LocalizeAndSetText(
		"TXT_KEY_OVERLAY_FILTER_JFD_PLAYER_SHOW_PLAYER", Locale.ToUpper(strCivDesc))
		Controls.OverlayMapFilterPlayerPullDown:GetButton():LocalizeAndSetToolTip(
		"TXT_KEY_OVERLAY_FILTER_JFD_PLAYER_SHOW_PLAYER_TT")
	else
		Controls.OverlayMapFilterPlayerPullDown:GetButton():LocalizeAndSetText(
		"TXT_KEY_OVERLAY_FILTER_JFD_PLAYER_SHOW_ALL")
		Controls.OverlayMapFilterPlayerPullDown:GetButton():LocalizeAndSetToolTip(
		"TXT_KEY_OVERLAY_FILTER_JFD_PLAYER_SHOW_ALL_TT")
	end
