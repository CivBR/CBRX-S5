...
function RefreshAdditionalInformationEntries()
	...
	local additionalEntries = {
		{
			text = Locale.Lookup("TXT_KEY_ADVISOR_COUNSEL"),
			call = function()
				Popup(ButtonPopupTypes
					.BUTTONPOPUP_ADVISOR_COUNSEL);
			end
		},
		{
			text = Locale.Lookup("TXT_KEY_ADVISOR_SCREEN_TECH_TREE_DISPLAY"),
			call = function()
				Popup(
					ButtonPopupTypes.BUTTONPOPUP_TECH_TREE, nil, -1);
			end
		},
		{
			text = Locale.Lookup("TXT_KEY_DIPLOMACY_OVERVIEW"),
			call = function()
				Popup(ButtonPopupTypes
		...
		{
			text = Locale.Lookup("TXT_KEY_MILITARY_OVERVIEW"),
			call = function()
				Popup(ButtonPopupTypes
		...
		{
			text = Locale.Lookup("TXT_KEY_ECONOMIC_OVERVIEW"),
			call = function()
				Popup(ButtonPopupTypes
		...
		{
			text = Locale.Lookup("TXT_KEY_VP_TT"),
			call = function()
				Popup(ButtonPopupTypes
		...
		{
			text = Locale.Lookup("TXT_KEY_DEMOGRAPHICS"),
			call = function()
				Popup(ButtonPopupTypes
		...
		{
			text = Locale.Lookup("TXT_KEY_POP_NOTIFICATION_LOG"),
			call = function()
				Popup(
					ButtonPopupTypes.BUTTONPOPUP_NOTIFICATION_LOG, Game.GetActivePlayer());
			end
		},
		{
			text = Locale.Lookup("TXT_KEY_TRADE_ROUTE_OVERVIEW"),
			call = function()
				Popup(ButtonPopupTypes
					.BUTTONPOPUP_TRADE_ROUTE_OVERVIEW);
			end
		},
	};
...
function UpdateNewData(playerID, szTag)
	...
	if (not bIsMinor) and player:IsAlive() then
		...
		local strScoreShortDesc = Locale.ConvertTextKey("[{1_Color}]{2_Num}[ENDCOLOR]", colourScore,
			tostring(numScoreRank))
		local strScoreDesc = Locale.ConvertTextKey("Rank: [{3_Colour}]{1_Desc}. Score: {2_Num}[ENDCOLOR]", numScoreRank,
			numScore, colourScore)
		...
		if pCapital then
			...
			elseif pPlayer:IsGoldenAge() then
				...
				strHappShortDesc = Locale.ConvertTextKey("[COLOR_JFD_OVERLAY_GOLDEN_AGE]{1_Num}[ENDCOLOR]",
			...
			elseif Player.IsDarkAge and pPlayer:IsDarkAge() then
				...
				strHappShortDesc = Locale.ConvertTextKey("[COLOR_JFD_OVERLAY_DARK_AGE]{1_Num}[ENDCOLOR]",
					pPlayer:GetGoldenAgeTurns())
			...
			elseif pPlayer:IsAnarchy() then
				...
				strHappShortDesc = Locale.ConvertTextKey("[COLOR_JFD_OVERLAY_ANARCHY]{1_Num}[ENDCOLOR]",
			...
			if ideologyID ~= -1 then
				...
				local strIdeoFont = "[" ..
					Locale.Substring(policyBranchType, 14, Locale.Length(policyBranchType) - 14) .. "]"
				...
				if civRelationsCount < 10 then
					...
					if pOtherPlayer:IsAlive() and (not pOtherPlayer:IsMinorCiv()) and iOtherPlayer ~= iPlayerLoop and pTeam:IsHasMet(iOtherTeam) then
						if pTeam:IsAtWar(iOtherTeam) then
							...
							CivIconHookup(iOtherPlayer, 32, instance.RelationsCivIcon, instance.RelationsCivIconBG,
								instance.RelationsCivIconShadow, false, true, instance.RelationsCivIconHighlight)
...
function ShowHideMainButton()
	if Controls.MainButton:IsHidden() then
		...
		Controls.MainButton:SetSelected(true)
	else
		Controls.MainButton:SetHide(true)
		Controls.MainButton:SetSelected(false)
	end
end

-------------------------------------------------
-------------------------------------------------
local g_PlayerListInstanceManager = InstanceManager:new("PlayerEntryInstance", "PlayerEntryBox", Controls
	.PlayerListStack);
function OnWorldCivsListUpdated()
	...
	for _, worldCiv in pairs(worldCivsTable) do
		...
		if cyclePowerID ~= -1 then
			...
			strGovtStatsName = "[COLOR_JFD_VIRTUE][ICON_BULLET][ICON_JFD_CYCLE_OF_POWER] " ..
				Locale.ToUpper(cyclePower.Description) .. "[ENDCOLOR]"
		else
		...
		if Player.GetCurrentGovernment then
			...
			strGovtStatsName = strGovtStatsName ..
				"[COLOR_JFD_SOVEREIGNTY][ICON_BULLET][ICON_JFD_GOVERNMENT] " ..
				Locale.ConvertTextKey(GameInfo.JFD_Governments[governmentID].Description) .. "[ENDCOLOR]"
			-- strGovtStatsName = strGovtStatsName .. " (" .. srGovtName .. ")"

			local factionID = pPlayer:GetDominantFaction()
			if factionID ~= -1 then
				strGovtStatsName = strGovtStatsName ..
					"[COLOR_JFD_SOVEREIGNTY][ICON_BULLET]" ..
					GameInfo.JFD_Factions[factionID].IconString ..
					" " .. Locale.ConvertTextKey(GameInfo.JFD_Factions[factionID].Adjective) .. "[ENDCOLOR]"
		...
		if ideologyID ~= -1 then
			strGovtStatsName = strGovtStatsName ..
				"[COLOR_MAGENTA][ICON_BULLET]" ..
				ideologyFont ..
				" " .. Locale.ConvertTextKey(GameInfo.PolicyBranchTypes[ideologyID].Description) .. "[ENDCOLOR]	"
		end
		...
		if religionID >= 0 and pPlayer:HasCreatedPantheon() then
			...
			strGovtStatsName = strGovtStatsName ..
				"[COLOR_WHITE][ICON_BULLET]" ..
				religionFont .. Locale.ConvertTextKey(GameInfo.Religions[religionID].Description) .. "[ENDCOLOR]"
		end

		--STABILITY
		--Update status
		controlTable.PlayerStabilityStatsText:LocalizeAndSetText("[ICON_HAPPINESS_1] [COLOR_HAPPINESS]STABLE[ENDCOLOR]")
		controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
			"[ICON_HAPPINESS_1] [COLOR_HAPPINESS]STABLE[ENDCOLOR]")
		if pPlayer:IsEmpireSuperUnhappy() then
			controlTable.PlayerStabilityStatsText:LocalizeAndSetText(
				"[ICON_HAPPINESS_4] [COLOR_NEGATIVE_TEXT]CIVIL WAR![ENDCOLOR]")
			controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
				"[ICON_HAPPINESS_4] [COLOR_NEGATIVE_TEXT]CIVIL WAR![ENDCOLOR]")
		end
		...
		if pPlayer:IsEmpireUnhappy() then
			controlTable.PlayerStabilityStatsText:LocalizeAndSetText(
				"[ICON_HAPPINESS_3] [COLOR_UNHAPPINESS]REBELLION![ENDCOLOR]")
			controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
				"[ICON_HAPPINESS_3] [COLOR_UNHAPPINESS]REBELLION![ENDCOLOR]")
		end
		if pPlayer:IsGoldenAge() then
			controlTable.PlayerStabilityStatsText:LocalizeAndSetText(
				"[ICON_GOLDEN_AGE] [COLOR_GOLDEN_AGE]GOLDEN AGE![ENDCOLOR]")
			controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
				"[ICON_GOLDEN_AGE] [COLOR_GOLDEN_AGE]GOLDEN AGE![ENDCOLOR]")
		end
		if Player.IsDarkAge and pPlayer:IsDarkAge() then
			controlTable.PlayerStabilityStatsText:LocalizeAndSetText(
				"[ICON_DARK_AGE] [COLOR_DARK_AGE]DARK AGE![ENDCOLOR]")
			controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
				"[ICON_DARK_AGE] [COLOR_DARK_AGE]DARK AGE![ENDCOLOR]")
		end
		if pPlayer:IsAnarchy() then
			...
			controlTable.PlayerStabilityStatsText:LocalizeAndSetToolTip(
				"[ICON_RESISTANCE] [COLOR_RED]ANARCHY![ENDCOLOR]")
		end
		...
		controlTable.PlayerEcoStatsText:LocalizeAndSetToolTip("TXT_KEY_JFD_WORLD_CIVILIZATIONS_ECO_STATS", numPlotPercent,
			numPopPercent, numMilitaryPercent)
...
GameEvents.PlayerDoTurn.Add(function(playerID)
	g_iPlayerForView = playerID
	UpdateData()
end)

function UpdateData()
	local iPlayerID = g_iPlayerForView;
	...
	if (iPlayerID >= 0) then
		...
		if (pPlayer:IsTurnActive()) and (pPlayer:GetNumCities() > 0) then
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
				...
				if (pPlayer:GetGoldenAgeTurns() > 0) then
					...
					strGoldenAgeStr = string.format("%i/%i", pPlayer:GetGoldenAgeProgressMeter(),
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
				...
				if (pPlayer:GetNextPolicyCost() > 0) then
					strCultureStr = string.format("%i/%i (+%i)", pPlayer:GetJONSCulture(),
						pPlayer:GetNextPolicyCost(),
						pPlayer:GetTotalJONSCulturePerTurn());
				else
					strCultureStr = string.format("%i (+%i)", pPlayer:GetJONSCulture(),
						pPlayer:GetTotalJONSCulturePerTurn());
				end
		...
		if (pPlayer:IsUsingMayaCalendar()) then
			...
			local toolTipString = Locale.ConvertTextKey("TXT_KEY_MAYA_DATE_TOOLTIP",
...
function GoldTipHandler(control)
	...
	local fTotalIncome = fGoldPerTurnFromCities + iGoldPerTurnFromOtherPlayers + fCityConnectionGold +
		iGoldPerTurnFromReligion + fTradeRouteGold + fTraitGold;
	...
	if (iGoldPerTurnFromOtherPlayers > 0) then
		strText = strText ..
			"[NEWLINE]  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_OTHERS", iGoldPerTurnFromOtherPlayers);
	end
	if (iGoldPerTurnFromReligion > 0) then
		strText = strText ..
			"[NEWLINE]  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_RELIGION", iGoldPerTurnFromReligion);
	end
	...
	if (fTotalIncome + iTotalGold < 0) then
		strText = strText ..
			"[NEWLINE][COLOR:255:60:60:255]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_LOSING_SCIENCE_FROM_DEFICIT") .. "[/COLOR]";
	end
...
function HappinessTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		local iHandicapHappiness = pPlayer:GetHappiness() - iPoliciesHappiness - iResourcesHappiness - iCityHappiness -
			...
			iExtraHappinessPerCity - iLeagueHappiness;
		...
		if (pPlayer:IsEmpireVeryUnhappy()) then
			...
			strText = strText .. "[NEWLINE][NEWLINE]";
			strText = strText ..
				"[COLOR:255:60:60:255]" .. Locale.ConvertTextKey("TXT_KEY_TP_EMPIRE_VERY_UNHAPPY") .. "[/COLOR]";
		elseif (pPlayer:IsEmpireUnhappy()) then
			strText = strText .. "[NEWLINE][NEWLINE]";
			strText = strText ..
				"[COLOR:255:60:60:255]" .. Locale.ConvertTextKey("TXT_KEY_TP_EMPIRE_UNHAPPY") .. "[/COLOR]";
		end
		...
		if (iNaturalWonderHappiness ~= 0) then
			...
			strText = strText ..
				"  [ICON_BULLET]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_NATURAL_WONDERS", iNaturalWonderHappiness);
		end
		...
		if (iMinorCivHappiness ~= 0) then
			...
			strText = strText ..
				"  [ICON_BULLET]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_CITY_STATE_FRIENDSHIP", iMinorCivHappiness);
		end
		...
		if (iUnhappinessPublicOpinion > 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_PUBLIC_OPINION", iUnhappinessPublicOpinion);
		end
		strText = strText .. "[/COLOR]";
...
function CultureTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		local iCultureFromGoldenAge = pPlayer:GetTotalJONSCulturePerTurn() - iCultureForFree - iCultureFromCities -
			iCultureFromHappiness - iCultureFromMinors - iCultureFromReligion - iCultureFromTraits -
			iCultureFromBonusTurns;
