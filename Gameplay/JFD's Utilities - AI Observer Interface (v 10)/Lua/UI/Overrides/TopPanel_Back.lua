...
function UpdateNewData(playerID, szTag)
	...
	if (player:IsUsingMayaCalendar()) then
		...
		local toolTipString = Locale.ConvertTextKey("TXT_KEY_MAYA_DATE_TOOLTIP", player:GetMayaCalendarLongString(),
			traditionalDate);
	else
	...
	if (not bIsMinor) and player:IsAlive() then
		...
		if currentReligionID > -1 and player:GetCapitalCity() and Game.GetHolyCityForReligion(currentReligionID, playerID) then
			currentReligionDesc = Locale.ConvertTextKey(
			"[ICON_RELIGION]Religion: [COLOR_WHITE] {2_ReliFont}{1_ReliDesc}[ENDCOLOR]",
				Game.GetReligionName(currentReligionID), GameInfo.Religions[currentReligionID].IconString)
		...
		if currentGovernmentID > -1 then
			...
			govermentDesc = Locale.ConvertTextKey(
			"Government: [COLOR_JFD_SOVEREIGNTY] {1_GovtDesc} {2_PowerDesc}[ENDCOLOR]", government.Description, powerDesc)
			if currentGovernmentID == governmentTheocracyID then
				politicDesc = Locale.ConvertTextKey(
				"Dominant: [COLOR_JFD_SOVEREIGNTY_FADING] {2_FontIcon}{1_PoliticDesc}[ENDCOLOR]",
					JFD_GetPoliticDescription(playerID, currentReligionID, true), "[ICON_JFD_CLERGY]")
			...
			elseif currentPoliticID and currentPoliticID > -1 then
				...
				politicDesc = Locale.ConvertTextKey(
				"Dominant: [COLOR_JFD_SOVEREIGNTY_FADING] {3_FontIcon} {1_PoliticDesc} ({2_ShortDesc})[ENDCOLOR]",
					JFD_GetPoliticDescription(playerID, currentPoliticID), politicInfo.ShortDescription,
					politicInfo.FontIcon)
				Controls.Politics:SetHide(false)
				Controls.Politics:LocalizeAndSetText(politicDesc)
			end
		end
		...
		if currentResearchID > -1 then
			currentResearchDesc = Locale.ConvertTextKey("[ICON_RESEARCH]Research: [COLOR_CYAN] {1_TechDesc}[ENDCOLOR]",
				GameInfo.Technologies[currentResearchID].Description)
		end
		...
		local currentPopulationDesc = Locale.ConvertTextKey(
		"[ICON_CITIZEN]Population: [COLOR_GREEN] {1_Num} ({2_Num})[ENDCOLOR]", currentPopulation, currentPopulationRank)
		...
		local currentCitiesDesc = Locale.ConvertTextKey(
		"[ICON_CITY_STATE]Cities: [COLOR_BUILDING_TEXT] {1_Num} ({2_Num})[ENDCOLOR]", currentCities, currentCitiesRank)
		...
		local currentManpowerDesc = Locale.ConvertTextKey(
		"[ICON_STRENGTH]Manpower: [COLOR_UNIT_TEXT] {1_Num} ({2_Num})[ENDCOLOR]", currentManpower, currentManpowerRank)
		...
		if currentIdeologyID > -1 then
			...
			currentIdeologyDesc = Locale.ConvertTextKey(
			"[ICON_CULTURE]Ideology: [COLOR_MAGENTA] {2_IdeologyFont} {1_IdeologyDesc}[ENDCOLOR]",
				GameInfo.PolicyBranchTypes[currentIdeologyID].Description, ideologyFont[currentIdeologyID])
			Controls.CurrentIdeology:SetText(currentIdeologyDesc)
		else
			currentIdeologyID = player:GetDominantPolicyBranchForTitle()
			if currentIdeologyID > -1 then
				currentIdeologyDesc = Locale.ConvertTextKey(
				"[ICON_CULTURE]Ideology: [COLOR_MAGENTA] {1_IdeologyDesc}[ENDCOLOR]",
					GameInfo.PolicyBranchTypes[currentIdeologyID].Description)
			end
			Controls.CurrentIdeology:SetText(currentIdeologyDesc)
		end
		...
		for city in player:Cities() do
			...
			if currentBuildingProductionID > -1 then
				...
				if buildingClass.MaxPlayerInstances == 1 or buildingClass.MaxGlobalInstances == 1 then
					currentWonderDesc = Locale.ConvertTextKey(
					"[ICON_CAPITAL]Wonder: [COLOR_CITY_BROWN]{1_BuildingDesc}[ENDCOLOR]", building.Description)
					break
				end
			end
		end
		...
		for otherPlayerID = 0, GameDefines.MAX_MINOR_CIVS - 1 do
			...
			if otherPlayer:IsAlive() then
				if otherPlayer:IsAtWarWith(playerID) then
					...
					CivIconHookup(otherPlayerID, 32, instance.WarCivIcon, instance.WarCivIconBG,
						instance.WarCivIconShadow, false, true, instance.WarCivIconHighlight)
				...
				if otherPlayer:IsDoF(playerID) then
					...
					CivIconHookup(otherPlayerID, 32, instance.FriendsCivIcon, instance.FriendsCivIconBG,
						instance.FriendsCivIconShadow, false, true, instance.FriendsCivIconHighlight)
				...
				if otherPlayer:IsMinorCiv() and otherPlayer:IsAllies(playerID) then
					...
					CivIconHookup(otherPlayerID, 32, instance.AlliesCivIcon, instance.AlliesCivIconBG,
						instance.AlliesCivIconShadow, false, true, instance.AlliesCivIconHighlight)
...
function UpdateData()
	...
	if (iPlayerID >= 0) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
				...
				if (sciencePerTurn <= 0) then
					strScienceText = string.format("[COLOR:255:60:60:255]" ..
					Locale.ConvertTextKey("TXT_KEY_NO_SCIENCE") .. "[/COLOR]");
					-- We have science
				else
			...
			if userSettingProsperityCore then
				local strProsperityStr = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_JFD_PROSPERITY",
					JFD_GetProsperityScore(iPlayerID))
			...
			local strInternationalTradeRoutes = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_INTERNATIONAL_TRADE_ROUTES",
				iUsedTradeRoutes, iAvailableTradeRoutes);
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
				...
				if (pPlayer:GetGoldenAgeTurns() > 0) then
					if (pPlayer:GetGoldenAgeTourismModifier() > 0) then
						strGoldenAgeStr = string.format(
						Locale.ToUpper(Locale.ConvertTextKey("TXT_KEY_UNIQUE_GOLDEN_AGE_ANNOUNCE")) .. " (%i)",
							pPlayer:GetGoldenAgeTurns());
					else
						strGoldenAgeStr = string.format(
						Locale.ToUpper(Locale.ConvertTextKey("TXT_KEY_GOLDEN_AGE_ANNOUNCE")) .. " (%i)",
							pPlayer:GetGoldenAgeTurns());
					end
				else
					local goldenAgePointsPerTurn = JFD_GetGoldenAgePointsPerTurn(iPlayerID) +
					JFD_GetYieldAddins("YIELD_GOLDEN_AGE"):YieldSum()
					if goldenAgePointsPerTurn >= 0 then
						strGoldenAgeStr = string.format("%i/%i (+%i)", pPlayer:GetGoldenAgeProgressMeter(),
							pPlayer:GetGoldenAgeProgressThreshold(), goldenAgePointsPerTurn);
					else
						strGoldenAgeStr = string.format("%i/%i ([COLOR_WARNING_TEXT]%i[ENDCOLOR])",
							pPlayer:GetGoldenAgeProgressMeter(), pPlayer:GetGoldenAgeProgressThreshold(),
							goldenAgePointsPerTurn);
					end
				end
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
				...
				if (pPlayer:GetNextPolicyCost() > 0) then
					strCultureStr = string.format("%i/%i (+%i)", pPlayer:GetJONSCulture(), pPlayer:GetNextPolicyCost(),
						iCulturePerTurn);
				else
			...
			if userSettingSovereigntyCore then
				if pPlayer:HasGovernment() then
					...
					strSovereigntyStr = Locale.ConvertTextKey("TXT_KEY_JFD_SOVEREIGNTY_TOP", pSovereigntyColour,
						iSovereignty, JFD_GetMaxSovereignty(iPlayerID))
		...
		if (iUnitSupplyMod ~= 0) then
			...
			local strUnitSupplyToolTip = Locale.ConvertTextKey("TXT_KEY_UNIT_SUPPLY_REACHED_TOOLTIP", iUnitsSupplied,
				iUnitsOver, -iUnitSupplyMod);
...
function ScienceTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
		...
		if (iScienceFromBudgetDeficit ~= 0) then
			...
			strText = strText ..
			Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_FROM_BUDGET_DEFICIT", iScienceFromBudgetDeficit / 100);
			strText = strText .. "[NEWLINE]";
		end
		...
		if (iScienceFromRAs ~= 0) then
			...
			strText = strText ..
			Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_FROM_RESEARCH_AGREEMENTS", iScienceFromRAs / 100);
		end
		...
		if (iScienceChange ~= 0) then
			...
			if (iScienceChange > 0) then
				strText = strText ..
				Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_GAINED_FROM_HAPPINESS", iScienceChange / 100);
			else
				strText = strText ..
				Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_LOST_FROM_UNHAPPINESS", iScienceChange / 100);
			end
		end
		--END
		--===============================
...
function GoldTipHandler(control)
	...
	local fTradeRouteGold = (pPlayer:GetGoldFromCitiesTimes100() - pPlayer:GetGoldFromCitiesMinusTradeRoutesTimes100()) /
	100;
	...
	iGoldPerTurnFromReligion + fTradeRouteGold + fTraitGold + iMinorGold + iGoldChange + iInternalRouteGold;
	...
	strText = strText ..
	"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_CITY_OUTPUT", fGoldPerTurnFromCities);
	strText = strText ..
	"[NEWLINE]  [ICON_BULLET]" ..
	Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_CITY_CONNECTIONS", math.floor(fCityConnectionGold));
	strText = strText ..
	"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_ITR", math.floor(fTradeRouteGold));
	if (math.floor(fTraitGold) > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_TRAITS", math.floor(fTraitGold));
	end
	--CBP
	if (iInternalRouteGold > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_INTERNAL_TRADE", iInternalRouteGold);
	end
	if (iGoldPerTurnFromOtherPlayers > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_OTHERS", iGoldPerTurnFromOtherPlayers);
	end
	if (iGoldPerTurnFromReligion > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_RELIGION", iGoldPerTurnFromReligion);
	end
	-- C4DF
	-- Gold from Vassals
	if (iGoldFromVassals > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_VASSALS", iGoldFromVassals);
	end

	-- Gold from Vassal Tax
	if (iGoldFromVassalTax > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_VASSAL_TAX", iGoldFromVassalTax);
	end
	--  END
	-- COMMUNITY PATCH CHANGE
	if (iMinorGold > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_MINORS", iMinorGold);
	end
	-- Gold gained from happiness
	if (iGoldChange > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_GAINED_FROM_HAPPINESS", iGoldChange);
	end
	--END
	--===============================
	--JFD's Top Panel Addins BEGINS
	...
	-- END C4DF
	local iTotalExpenses = iUnitCost + iUnitSupply + iBuildingMaintenance + iImprovementMaintenance +
	iGoldPerTurnToOtherPlayers + iContractMaintenance;
	-- BEGIN C4DF
	...
	if (iUnitSupply ~= 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_UNIT_SUPPLY", iUnitSupply);
	end
	if (iBuildingMaintenance ~= 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_BUILDING_MAINT", iBuildingMaintenance);
	end
	if (iContractMaintenance ~= 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_CONTRACT_MAINT", iContractMaintenance);
	end
	if (iImprovementMaintenance ~= 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_TILE_MAINT", iImprovementMaintenance);
	end
	if (iGoldPerTurnToOtherPlayers > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_TO_OTHERS", iGoldPerTurnToOtherPlayers);
	end
	-- COMMUNITY PATCH CHANGE
	...
	if (iGoldChange < 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_LOST_FROM_UNHAPPINESS", (iGoldChange * -1));
	end
	--END
	-- C4DF
	if (iVassalMaintenance > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_VASSAL_MAINT", iVassalMaintenance);
	end

	-- Gold from Vassal Tax
	if (iExpenseFromVassalTaxes > 0) then
		strText = strText ..
		"[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TOP_GOLD_VASSAL_TAX", iExpenseFromVassalTaxes);
	end
	--  END
	--===============================
	--JFD's Top Panel Addins BEGINS
	...
	if (fTotalIncome + iTotalGold < 0) then
		strText = strText ..
		"[NEWLINE][COLOR:255:60:60:255]" .. Locale.ConvertTextKey("TXT_KEY_TP_LOSING_SCIENCE_FROM_DEFICIT") .. "[/COLOR]";
	end
...
function HappinessTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		-- END
		-- CBP Edit
		local iHandicapHappiness = pPlayer:GetHappiness() - iPoliciesHappiness - iResourcesHappiness - iCityHappiness -
		iBuildingHappiness - iTradeRouteHappiness - iReligionHappiness - iNaturalWonderHappiness - iMinorCivHappiness -
		iExtraHappinessPerCity - iLeagueHappiness - iHappinessFromBonusResources - iHappinessFromMonopoly -
		iHappinessFromEvent;
		...
		if (pPlayer:IsEmpireVeryUnhappy()) then
			if (pPlayer:IsEmpireSuperUnhappy()) then
				strText = strText .. "[NEWLINE][NEWLINE]";
				strText = strText ..
				"[COLOR:255:60:60:255]" .. Locale.ConvertTextKey("TXT_KEY_TP_EMPIRE_SUPER_UNHAPPY") .. "[/COLOR]";
			end

			strText = strText .. "[NEWLINE][NEWLINE]";
			strText = strText ..
			"[COLOR:255:60:60:255]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_EMPIRE_VERY_UNHAPPY", iTestHappiness, iMilitaryHappiness) .. "[/COLOR]";
		elseif (pPlayer:IsEmpireUnhappy()) then
			strText = strText .. "[NEWLINE][NEWLINE]";
			strText = strText ..
			"[COLOR:255:60:60:255]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_EMPIRE_UNHAPPY", iTestHappiness, iMilitaryHappiness) .. "[/COLOR]";
		end
		...
		local iTotalHappiness = iPoliciesHappiness + iResourcesHappiness + iCityHappiness + iBuildingHappiness +
		iMinorCivHappiness + iHandicapHappiness + iTradeRouteHappiness + iReligionHappiness + iNaturalWonderHappiness +
		iExtraHappinessPerCity + iLeagueHappiness + iHappinessFromBonusResources + iHappinessFromMonopoly +
		iHappinessFromEvent;
		...
		strText = strText .. "[NEWLINE]";
		strText = strText ..
		"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_FROM_RESOURCES", iResourcesHappiness);
		...
		for resource in GameInfo.Resources() do
			...
			if (iHappiness > 0) then
				...
				strText = strText ..
				"          +" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_EACH_RESOURCE", iHappiness, resource.IconString,
		...
		if (iHappinessFromEvent > 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_EVENT", iHappinessFromEvent);
		end
		...
		if isUsingCBP then
			if (iHappinessFromMonopoly > 0) then
				...
				strText = strText ..
				"  [ICON_BULLET]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_RESOURCE_MONOPOLY", iHappinessFromMonopoly);
			end
			if (iHappinessFromBonusResources > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_RESOURCE_POP_BONUS", iHappinessFromBonusResources);
			end
		end
		...
		if isUsingCBP then
			...
			if (iGetLuxuryBonusPlusOne > 0) then
				strText = strText ..
				"[NEWLINE][NEWLINE][ENDCOLOR]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_THRESHOLD_VALUE", iPopNeeded, iPopulation,
					Locale.ToNumber(((iGetLuxuryBonusPlusOne - iGetLuxuryBonus) / 100), "#.##"));
			end
		end
		-- END
		-- C4DF
		if (iHappinessFromVassals > 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"          +" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_VASSALS", iHappinessFromVassals);
		end
		-- END
		...
		if (iHappinessFromExtraResources > 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"          +" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_RESOURCE_VARIETY", iHappinessFromExtraResources);
		end
		...
		if (iExtraLuxuryHappiness >= 1) then
			...
			strText = strText ..
			"          +" ..
			Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_EXTRA_PER_RESOURCE", iExtraLuxuryHappiness,
				iNumHappinessResources);
		end

		-- Misc Happiness from Resources
		local iMiscHappiness = iResourcesHappiness - iBaseHappinessFromResources - iHappinessFromExtraResources -
		(iExtraLuxuryHappiness * iNumHappinessResources);
		if (iMiscHappiness > 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"          +" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_OTHER_SOURCES", iMiscHappiness);
		end
		...
		if (iPoliciesHappiness >= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_POLICIES", iPoliciesHappiness);
		end
		strText = strText .. "[NEWLINE]";
		strText = strText ..
		"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_BUILDINGS", iBuildingHappiness);
		if (iTradeRouteHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_CONNECTED_CITIES", iTradeRouteHappiness);
		end
		if (iReligionHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_STATE_RELIGION", iReligionHappiness);
		end
		if (iNaturalWonderHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_NATURAL_WONDERS", iNaturalWonderHappiness);
		end
		if (iExtraHappinessPerCity ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_CITY_COUNT", iExtraHappinessPerCity);
		end
		if (iMinorCivHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_CITY_STATE_FRIENDSHIP", iMinorCivHappiness);
		end
		if (iLeagueHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_LEAGUES", iLeagueHappiness);
		end
		strText = strText .. "[NEWLINE]";
		strText = strText ..
		"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_DIFFICULTY_LEVEL", iHandicapHappiness);
		strText = strText .. "[/COLOR]";
		...
		if (iUnhappinessFromCityCount ~= "0") then
			...
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_CITY_COUNT", iUnhappinessFromCityCount);
			-- CBP
		end
		-- END
		if (iUnhappinessFromCapturedCityCount ~= "0") then
			...
			strText = strText ..
			"  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_CAPTURED_CITY_COUNT", iUnhappinessFromCapturedCityCount);
		end
		-- COMMUNITY PATCH CHANGES BELOW
		if (iUnhappinessFromPop ~= "0") then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_POPULATION", iUnhappinessFromPop);
		end
		--END
		...
		if (iUnhappinessFromPupetCities > 0) then
			...
			strText = strText ..
			"  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_PUPPET_CITIES", iUnhappinessFromPupetCities / 100);
		end

		if (unhappinessFromSpecialists > 0) then
			strText = strText ..
			"[NEWLINE]  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_SPECIALISTS", unhappinessFromSpecialists / 100);
		end
		...
		if (iUnhappinessFromOccupiedCities ~= "0") then
			...
			strText = strText ..
			"  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_OCCUPIED_POPULATION", iUnhappinessFromOccupiedCities);
		end
		if (iUnhappinessFromUnits ~= "0") then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_UNITS", iUnhappinessFromUnits);
		end
		if (iPoliciesHappiness < 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText ..
		...
		if (iUnhappinessPublicOpinion > 0) then
			...
			strText = strText ..
			"  [ICON_BULLET]" ..
			Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_PUBLIC_OPINION", iUnhappinessPublicOpinion);
		end
		...
		if isUsingCBP then
			if (iUnhappinessFromStarving > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_STARVING", iUnhappinessFromStarving);
			end
			if (iUnhappinessFromPillaged > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_PILLAGED", iUnhappinessFromPillaged);
			end
			if (iUnhappinessFromGold > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_GOLD", iUnhappinessFromGold);
			end
			if (iUnhappinessFromDefense > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_DEFENSE", iUnhappinessFromDefense);
			end
			if (iUnhappinessFromConnection > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_CONNECTION", iUnhappinessFromConnection);
			end
			if (iUnhappinessFromMinority > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_MINORITY", iUnhappinessFromMinority);
			end
			if (iUnhappinessFromScience > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_SCIENCE", iUnhappinessFromScience);
			end
			if (iUnhappinessFromCulture > 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText ..
				"  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_UNHAPPINESS_CULTURE", iUnhappinessFromCulture);
			end
		end
		--END CHANGES
		strText = strText .. "[/COLOR]";
...
function GoldenAgeTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		if (pPlayer:GetGoldenAgeTurns() > 0) then
			...
			strText = Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_PROGRESS", pPlayer:GetGoldenAgeProgressMeter(),
			...
			if (iHappiness >= 0) then
				...
				strText = strText ..
				"[COLOR_WARNING_TEXT]" ..
				Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_LOSS", -iHappiness) .. "[ENDCOLOR]";
			end
...
function CultureTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		if (iCultureFromBonusTurns ~= 0) then
			...
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			Locale.ConvertTextKey("TXT_KEY_TP_CULTURE_FROM_BONUS_TURNS", iCultureFromBonusTurns, iBonusTurns);
		end
		...
		local iCultureFromGoldenAge = pPlayer:GetTotalJONSCulturePerTurn() - iCultureForFree - iCultureFromCities -
		iCultureFromHappiness - iCultureFromMinors - iCultureFromReligion - iCultureFromTraits - iCultureFromBonusTurns -
		iCommunityCulture - iCultureFromVassals;                                                                                                                                                                                                                         -- last part added (COMMUNITY PATCH)
...
function FaithTipHandler(control)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		if (pPlayer:HasCreatedPantheon()) then
			if (Game.GetNumReligionsStillToFound() > 0 or pPlayer:HasCreatedReligion()) then
				if (pPlayer:GetCurrentEra() < GameInfo.Eras["ERA_INDUSTRIAL"].ID) then
					strText = strText ..
					Locale.ConvertTextKey("TXT_KEY_TP_FAITH_NEXT_PROPHET", pPlayer:GetMinimumFaithNextGreatProphet());
					strText = strText .. "[NEWLINE]";
					strText = strText .. "[NEWLINE]";
				end
			end
			...
			if (pPlayer:CanCreatePantheon(false)) then
				strText = strText ..
				Locale.ConvertTextKey("TXT_KEY_TP_FAITH_NEXT_PANTHEON", Game.GetMinimumFaithNextPantheon());
				strText = strText .. "[NEWLINE]";
			else
		...
		if (Game.GetNumReligionsStillToFound() < 0) then
			...
			strText = strText ..
			Locale.ConvertTextKey("TXT_KEY_TP_FAITH_RELIGIONS_LEFT", Game.GetNumReligionsStillToFound());
		end
		...
		if (pPlayer:GetCurrentEra() >= GameInfo.Eras["ERA_INDUSTRIAL"].ID) then
			...
			strText = strText .. "[NEWLINE]";
			strText = strText ..
			Locale.ConvertTextKey("TXT_KEY_TP_FAITH_NEXT_GREAT_PERSON", pPlayer:GetMinimumFaithNextGreatProphet());
	...
	for pResource in GameInfo.Resources() do
		...
		if (Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC) then
			...
			if (bShowResource) then
				...
				strText = strText ..
				iNumAvailable .. " " .. pResource.IconString .. " " .. Locale.ConvertTextKey(pResource.Description);
...
function InternationalTradeRoutesTipHandler(control)
	...
	if (iNumLandTradeUnitsAvail > 0) then
		...
		local strUnusedTradeUnitWarning = Locale.ConvertTextKey(
		"TXT_KEY_TOP_PANEL_INTERNATIONAL_TRADE_ROUTES_TT_UNASSIGNED", iNumLandTradeUnitsAvail,
			GameInfo.Units[iTradeUnitType].Description);
	...
	local strText = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_INTERNATIONAL_TRADE_ROUTES_TT", iUsedTradeRoutes,
		iAvailableTradeRoutes);
	strTT = strTT .. strText;
