...
-- logger:debug("Processing InfoAddictCivRelations");
...
function initIconPositions()
	...
	for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
		...
		if (not Players[iPlayerLoop]:IsMinorCiv() and Players[iPlayerLoop]:IsEverAlive()) then
			...
			logger:trace("iconPosition = " ..
				iconPosition .. ", thisIcon = " .. thisIcon .. ", iconDistance = " .. iconDistance);
		...
		if (hasMetCiv(thisPid) and Players[thisPid]:IsAlive()) then
			...
			for targetPid = thisPid + 1, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
				if (hasMetCiv(targetPid) and Players[targetPid]:IsAlive()) then
					...
					local thisName = Locale.ConvertTextKey(GameInfo.Civilizations[thisPlayer:GetCivilizationType()]
						.ShortDescription);
					local targetName = Locale.ConvertTextKey(GameInfo.Civilizations
						[targetPlayer:GetCivilizationType()].ShortDescription);
					...
					if (thisTeam:IsAtWar(targetTid) and isKeySelected(keyType.war)) then
						...
						if (thisTeam:IsDefensivePact(targetTid) and isKeySelected(keyType.defensive)) then
							...
							if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
								tooltipData["defensive"][thisPid] = tooltipData["defensive"][thisPid] ..
									tooltipPad .. targetName;
								tooltipData["defensive"][targetPid] = tooltipData["defensive"][targetPid] ..
									tooltipPad .. thisName;
							end;
						...
						if (isKeySelected(keyType.borders)) then
							if (thisTeam:IsAllowsOpenBordersToTeam(targetTid)) then
								if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
									tooltipData["borders"][thisPid] = tooltipData["borders"][thisPid] ..
										tooltipPad .. targetName;
								end;
							...
							if (targetTeam:IsAllowsOpenBordersToTeam(thisTid)) then
								if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
									tooltipData["borders"][targetPid] = tooltipData["borders"][targetPid] ..
										tooltipPad .. thisName;
								end;
						...
						if (isKeySelected(keyType.denounce)) then
							if (thisPlayer:IsDenouncedPlayer(targetPid)) then
								if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
									tooltipData["denounce"][thisPid] = tooltipData["denounce"][thisPid] ..
										tooltipPad .. targetName;
								end;
							...
							if (targetPlayer:IsDenouncedPlayer(thisPid)) then
								if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
									tooltipData["denounce"][targetPid] = tooltipData["denounce"][targetPid] ..
										tooltipPad .. thisName;
								end;
		...
		if (tooltipData["war"][thisPid] ~= nil) then
			...
			if (tooltipData["war"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					war_text ..
					Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONKEY_WAR") ..
					":[ENDCOLOR] " .. tooltipData["war"][thisPid];
			end;
			...
			if (tooltipData["defensive"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					defensivePact_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONKEY_DEFENSIVE_PACT") ..
			...
			if (tooltipData["DoF"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					DoF_text ..
					Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONKEY_DECLARATION_OF_FRIENDSHIP") ..
					":[ENDCOLOR] " .. tooltipData["DoF"][thisPid];
			end;
			...
			if (tooltipData["borders"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					openBorders_text ..
					Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONKEY_OPEN_BORDERS") ..
					":[ENDCOLOR] " .. tooltipData["borders"][thisPid];
			end;
			...
			if (tooltipData["denounce"][thisPid] ~= nil and tooltipData["denounce"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					denounce_text ..
					Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONKEY_DENOUNCEMENT") ..
					":[ENDCOLOR] " .. tooltipData["denounce"][thisPid];
			end;
...
function economicView()
			...
			if (repeatcheck == false and hackGuard < hackGuardPissed) then
				repeat
					...
					logger:trace("itemType: " ..
						tostring(itemType) ..
						",  duration: " .. tostring(duration) .. ",  finalTurn: " .. tostring(finalTurn) ..
						",  data1: " ..
						tostring(data1) .. ",  data2: " .. tostring(data2) .. ",  data3: " .. tostring(data3) ..
						",  flag1: " .. tostring(flag1) .. ",  fromPidDeal: " .. tostring(fromPidDeal));
					...
					if (fromCheck ~= thisPid and toCheck ~= thisPid) then
						logger:trace("Check fail:  fromCheck = " ..
							fromCheck .. ", toCheck = " .. toCheck .. ", thisPid = " .. thisPid);
						thisCheckFail = true;
					end;
					...
					if (itemType ~= nil and thisCheckFail == false) then
						repeat
							...
							if (toPid ~= fromPid) then
								...
								local toName = Locale.ConvertTextKey(GameInfo.Civilizations
									[toPlayer:GetCivilizationType()].ShortDescription);
								local fromName = Locale.ConvertTextKey(GameInfo.Civilizations
									[fromPlayer:GetCivilizationType()].ShortDescription);

								-- logger:debug("  Looking at trade from " ..
									-- fromName .. " to " .. toName .. " itemType = " .. itemType);
								...
								if (hasMetCiv(fromPid) and Players[fromPid]:IsAlive() and
									...
									if (TradeableItems.TRADE_ITEM_GOLD_PER_TURN == itemType) then
										...
										local exportstr = tooltipPad ..
											"[ICON_GOLD]" ..
											iconPad ..
											goldamount ..
											" " .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GPT_TO") ..
											" " .. toName .. " (" .. turnsLeft .. ")";
										local importstr = tooltipPad ..
											"[ICON_GOLD]" ..
											iconPad ..
											goldamount ..
											" " ..
											Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GPT_FROM") ..
											" " .. fromName .. " (" .. turnsLeft .. ")";
									...
									elseif (TradeableItems.TRADE_ITEM_RESEARCH_AGREEMENT == itemType) then
										local exportstr = tooltipPad ..
											"[ICON_RESEARCH]" .. iconPad .. " " .. toName .. " (" .. turnsLeft .. ")";
										...
										if (alreadyseen[exportstr .. fromName] ~= 1 and (isCivSelected(fromPid) or isCivSelected(toPid)) and isKeySelected(keyType.research)) then
											logger:trace("   " ..
												exportstr ..
												" [fromName = " ..
												fromName .. ", toName = " .. toName .. ", thisPid = " .. thisPid .. "]");
									...
									elseif (TradeableItems.TRADE_ITEM_RESOURCES == itemType) then
										...
										local exportstr = tooltipPad ..
											...
											iconPad ..
											resourceName ..
											" " .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_TO") ..
											" " .. toName .. " (" .. turnsLeft .. ")";
										local importstr = tooltipPad ..
											iconstr ..
											iconPad ..
											resourceName ..
											" " .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_FROM") ..
											" " .. fromName .. " (" .. turnsLeft .. ")";
					...
					if (hackGuard > hackGuardPissed) then
						-- logger:debug("SUP SUP, I'm the hack guard!  hackGuard = " ..
							-- hackGuard .. ", hackGuardPissed = " .. hackGuardPissed);
					end;
	...
	for thisPid = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
		if (hasMetCiv(thisPid) and Players[thisPid]:IsAlive()) then
			...
			for i = 1, table.count(tradeRoutesTable), 1 do
				...
				local toPlayer = Players[toPid];
				local toName = Locale.ConvertTextKey(GameInfo.Civilizations[toPlayer:GetCivilizationType()]
					.ShortDescription);
	...
	for thisPid = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
		if (hasMetCiv(thisPid) and Players[thisPid]:IsAlive()) then
			...
			if (exportstr ~= "") then
				str = str ..
					"[NEWLINE]" .. export_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_EXPORTS") ..
					":[ENDCOLOR] " .. exportstr;
			end;
			...
			if (importstr ~= "") then
				str = str ..
					"[NEWLINE]" .. import_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_IMPORTS") ..
					":[ENDCOLOR] " .. importstr;
			end;
			...
			if (traderoutesstr ~= "") then
				str = str ..
					"[NEWLINE]" ..
					traderoute_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_TRADEROUTES") ..
			...
			if (research[thisPid] ~= nil and research[thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					research_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_RESEARCH_AGREEMENTS") ..
			...
			for targetPid = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
				if (hasMetCiv(targetPid) and Players[targetPid]:IsAlive()) then
					...
					then
						...
						-- logger:debug(thisName .. " is hostile toward " .. targetName);
					elseif (currentAttitude == MajorCivApproachTypes.MAJOR_CIV_APPROACH_GUARDED and isKeySelected(keyType.guarded)) then
						...
						-- logger:debug(thisName .. " is guarded toward " .. targetName);
					elseif (currentAttitude == MajorCivApproachTypes.MAJOR_CIV_APPROACH_AFRAID and isKeySelected(keyType.afraid)) then
						...
						-- logger:debug(thisName .. " is afraid of " .. targetName);
					elseif (currentAttitude == MajorCivApproachTypes.MAJOR_CIV_APPROACH_FRIENDLY and isKeySelected(keyType.friendly)) then
						...
						if (isCivSelected(thisPid) or isCivSelected(targetPid)) then
							tooltipData["friendly"][thisPid] = tooltipData["friendly"][thisPid] ..
								tooltipPad .. targetName;
						end;
						-- logger:debug(thisName .. " is friendly toward " .. targetName);
					elseif (currentAttitude == MajorCivApproachTypes.MAJOR_CIV_APPROACH_NEUTRAL and isKeySelected(keyType.neutral)) then
						...
						-- logger:debug(thisName .. " is neutral toward " .. targetName);
					end;
				end;
			end;
		end;
	...
	for thisPid = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
		if (tooltipData["hostile"][thisPid] ~= nil) then
			...
			if (tooltipData["hostile"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" .. hostile_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_HOSTILE") ..
			...
			if (tooltipData["guarded"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" .. guarded_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GUARDED") ..
			...
			if (tooltipData["afraid"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" .. afraid_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_AFRAID") ..
			...
			if (tooltipData["friendly"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" ..
					friendly_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_FRIENDLY") ..
			...
			if (tooltipData["neutral"][thisPid] ~= "") then
				str = str ..
					"[NEWLINE]" .. neutral_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_NEUTRAL") ..
