...
function RankButtonHandler(rankType)
	...
	for rank, data in ipairs(rankTable) do
		...
		local civName = Locale.ConvertTextKey(GameInfo.Civilizations[Players[pid]:GetCivilizationType()]
		.ShortDescription);
...
function BuildEmpireStats()
	...
	local leader = GameInfo.Leaders("Type = '" ..
	GameInfo.Civilization_Leaders("CivilizationType = '" .. thisCiv.Type .. "'")().LeaderheadType .. "'")();
...
function ToggleStack(isRanking)
	if (isRanking == 1) then
		if (Controls.RankingStack:IsHidden()) then
			...
			Controls.RankingButton:SetText("[ICON_MINUS]" ..
			Locale.ConvertTextKey("TXT_KEY_INFOADDICT_MENUTITLE_WORLDRANKINGS"));
		else
			Controls.RankingStack:SetHide(true);
			Controls.RankingButton:SetText("[ICON_PLUS]" ..
			Locale.ConvertTextKey("TXT_KEY_INFOADDICT_MENUTITLE_WORLDRANKINGS"));
		end
	else
