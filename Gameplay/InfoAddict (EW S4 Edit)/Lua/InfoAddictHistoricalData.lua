...
local g_HoverBoxTooltipRowManager = InstanceManager:new("HoverBoxTooltipRowInstance", "HoverBoxTooltipRowStack",
	tipControlTable.HoverTooltipMainStack);
...
function DrawGraph(graphType)
  ...
  -- /EXPERIMENTAL CODE ]] --
	...
	-- civcount .. ", Points per Player = " .. pointsPerPlayer .. ", Points to graph = " .. pointsToGraph);
		...
		if (hasMetCiv(iPlayerLoop)) then
			...
			for iPoint = 0, pointsPerPlayer - 1, 1 do
				...
				if (previousValue ~= nil and value ~= nil and value ~= previousValue and smoove == false) then
					...
					local previousYOffset = math.floor((yMax / yRange) * (previousValue - yMinValue) + mainShiftY +
						pcount);
	...
	for i = 0, numLabels - 1, 1 do
		...
		-- logger:debug("X Labels: iPoint = " ..
		-- tostring(iPoint) .. ", thisturn = " .. tostring(thisTurn) .. ", turnText = " .. tostring(turnText));
...
function HoverBoxTooltipHandler(control)
	...
	local Title = "[COLOR_POSITIVE_TEXT]" ..
		getYearString(turnLabel) .. "  (" .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_TURN") ..
		" " .. turnLabel .. ")" .. "[ENDCOLOR]";
