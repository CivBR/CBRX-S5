...
function OnUnitCaptured(iByPlayer, iByUnit, iCapturedPlayer, iCapturedUnit, bWillBeKilled, iReason)
	...
	if (pGpUnit:IsGreatPerson()) then
		...
		local iExtra1 = (GameInfoTypes[GameInfo.UnitClasses[iClass].DefaultUnit] * 100 + iCapturedPlayer) * 100 +
			iByPlayer
		...
		if (bWillBeKilled) then
			if (iClass == iClassScientist) then
				...
				if (pByPlayer:IsHuman()) then
					local sHeading = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_HEADING", sGpDescription,
						pByUnit:GetName())
					local sMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_TEXT_BOOST", sGpDescription,
						pByUnit:GetName(), "[ICON_RESEARCH]", iBoost)
					pByPlayer:AddNotification(g_NotificationType, sMessage, sHeading, pByUnit:GetX(), pByUnit:GetY(),
						iExtra1, iByUnit)
				end
			elseif (iClass == iClassEngineer) then
				...
				if (pByPlayer:IsHuman()) then
					local sHeading = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_HEADING", sGpDescription,
						pByUnit:GetName())
					local sMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_TEXT_BOOST", sGpDescription,
						pByUnit:GetName(), "[ICON_PRODUCTION]", iBoost)
					pByPlayer:AddNotification(g_NotificationType, sMessage, sHeading, pByUnit:GetX(), pByUnit:GetY(),
						iExtra1, iByUnit)
				end
			elseif (iClass == iClassMerchant) then
				...
				if (pByPlayer:IsHuman()) then
					local sHeading = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_HEADING", sGpDescription,
						pByUnit:GetName())
					local sMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_TEXT_BOOST", sGpDescription,
						pByUnit:GetName(), "[ICON_GOLD]", iBoost)
					pByPlayer:AddNotification(g_NotificationType, sMessage, sHeading, pByUnit:GetX(), pByUnit:GetY(),
						iExtra1, iByUnit)
				end
			elseif (iClass == iClassGeneral or iClass == iClassAdmiral) then
				...
				if (pByPlayer:IsHuman()) then
					local sHeading = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_HEADING", sGpDescription,
						pByUnit:GetName())
					local sMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_TEXT_PROMOTION",
						sGpDescription, pByUnit:GetName())
					pByPlayer:AddNotification(g_NotificationType, sMessage, sHeading, pByUnit:GetX(), pByUnit:GetY(),
						iExtra1, iByUnit)
				end
			end
			...
			if (iClass == iClassArtist or iClass == iClassMusician or iClass == iClassWriter or iClass == iClassProphet) then
				...
				if (pByPlayer:IsHuman()) then
					local sHeading = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_HEADING", sGpDescription,
						pByUnit:GetName())
					local sMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATIONS_CAPTURE_GP_TEXT_UNIT", sGpDescription,
						pByUnit:GetName())
					pByPlayer:AddNotification(g_NotificationType, sMessage, sHeading, pByUnit:GetX(), pByUnit:GetY(),
						iExtra1, iByUnit)
				end
...
--Multiple capture title attachment bug fix
-- local sOP = " ("

-- function ExciseParenthesesFromUnitName(playerID, unitID)
-- 	local pPlayer = Players[playerID]
-- 	if not pPlayer then return end
-- 	local pUnit = pPlayer:GetUnitByID(unitID)
-- 	if (not pUnit) or pUnit:IsDead() then return end

-- 	local sFullName = pUnit:GetName()
-- 	local iOpenParenPosition = strfind(sFullName, sOP)
-- 	if not iOpenParenPosition then return end
-- 	local sTrimmedName = strsub(sFullName, iOpenParenPosition - 1)
-- 	pUnit:SetName(sTrimmedName)
-- end

-- Events.SerialEventUnitCreated.Add(ExciseParenthesesFromUnitName)
